...
import com.yyh.web.rest.CompanyResource;
import com.yyh.web.rest.vm.FileInfoVM;
import com.yyh.service.util.FileLock;
...
public class FileUploadService {
            ...
            try{
                ...
                if(chunks == files.size()){
                    ...
                    if(this.saveMd52FileMap(md5, outputFile.getName())){
                        log.debug("文件[" + md5 + "=>" + outputFile.getName() + "]保存关系到持久成失败，但并不影响文件上传，只会导致日后该文件可能被重复上传而已");
                    }
     ...
     * @return
     */
    public File getReadySpace(FileInfoVM info, String path){
